	...
	*/
	...
	//Markus sleep in every tent
		if ((s_player_sleep < 0) and (player distance _cursorTarget < 3)) then {
			...
			s_player_sleep = player addAction [localize "str_actions_self_sleep", "more\player_sleep.sqf",_cursorTarget, 0, false, true, "",""];
			//Markus
		};
	} else {
    ...
    if (_isMan and !_isAlive and !_isZombie and !_clothesTaken) then {
        if (s_player_clothes < 0) then {
            s_player_clothes = player addAction [("<t color='#0096ff'>")+("Nimm Kleidung")+("</t>"), "more\nlg_TakeClothes.sqf",[_cursorTarget], -10, false, true, "",""];
        };
    } else {
