/*
   ...
   *updated on 18.07.2014
*/
USE_CANBUILD = true;            // if you don't want to define your own positions
USE_TraderCity = false;  //      if you don't want to define your own positions (Epoch 1.0.5) // Shaun Riley - Value Change true -> flase
USE_POSITIONS = true;  // to use own positions and radius // Shaun Riley - Value Change false -> true
...
USE_SPEEDLIMIT = false;  // very secret function, nobody is supposed to find out what it does.
        SPEEDLIMIT = 55;                // Only used if USE_SPEEDLIMIT is true.
        ...
        {
                {
                        ...
                        if (_lSign) then
                        {
                                for '_i' from 0 to 360 step (270 / _radius)*2 do
                                {
                                        ...
                                        _object setDir _dir;
                                              _object setVehicleInit 'this setObjectTexture [0, ''SafeZones\sign.paa''];';
                                };
                        };
                } forEach _infiSZ;
        };
        ...
        [] spawn {
                ...
                while {1 == 1} do
                {
                        if (inSafezone) then
                        {
                                ...
                                {
                                        _obj = vehicle player;
                                        if ((_obj != player) && (player == driver _obj) && (abs speed _obj > _speedlimit)) then
                                        {
                                                _vel = velocity _obj;
                                                _x = 0.3;
